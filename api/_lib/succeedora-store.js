...
async function persistAiCreditTransaction(user, transaction, credits) {
  ...
  const entry = {
    ...
    fallbackUsed: transaction.fallbackUsed === true,
    creditsUsed: Math.max(0, Number(transaction.creditsUsed || (transaction.type === "usage" ? Math.abs(transaction.amount || 0) : 0))),
